<template>
<div>
  <Form v-show="showFormRule=='1'" ref="loginForm" :model="form" :rules="rules" @keydown.enter.native="handleSubmit">
  ...
  <Form v-show="showFormRule=='2'"  ref="loginFormRegister" :model="formRregister " :rules="registeRules" @keydown.enter.native="handleSubmitRregister" >
    <FormItem prop="phone">
    ...
    <FormItem prop="passwd">
      <Input type="password" v-model="formRregister.passwd" placeholder="请输入密码">
        <span slot="prepend">
          <Icon :size="14" type="md-lock"></Icon>
        </span>
      </Input>
    ...
    <FormItem prop="passwdCheck">
      <Input type="password" v-model="formRregister.passwdCheck" placeholder="请再次输入密码">
        <span slot="prepend">
          <Icon :size="14" type="md-lock"></Icon>
        </span>
      </Input>
  ...
  data () {
    const validatePass = (rule, value, callback) => {
        if (value === '') {
            ...
            if (this.formRregister.passwdCheck !== '') {
                // 对第二个密码框单独验证
                this.$refs.formRregister.validateField('passwdCheck');
            }
            callback();
        }
    };
    ...
    return {
      ...
      registeRules: {
        ...
        reusername: [{required: true, message: '请输入用户名', trigger: 'blur' }],
        passwd: [
